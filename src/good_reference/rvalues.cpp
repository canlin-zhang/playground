...
{
    // ----- prvalue (pure rvalue) -----
    // Prvalue is a temporary object that is not stored in a variable, so it doesn't have a stable address.
    // You can't compile this line, uncomment to see the error: "error: taking address of rvalue"
    // std::cout << "[prvalue] &std::string(1000, 'R'): "
    ...
    // std::string &s2 = std::string(1000, 'A');
    ...
    std::cout << "std::move is an xvalue expression because it casts s to an rvalue reference." << std::endl;

...
void demo_xvalue()
{
    ...
    auto t = std::move(s);
    std::cout << "std::move is an xvalue expression because it casts s to an rvalue reference, but "
                 "s is still a lvalue expression, just moved-from."
              << std::endl;
...
void demo_lvalue()
{
    ...
    // 1. It creates a named object "s" on stack (not on heap for this case due to small string
    // optimization)
    std::string s;
    s = "Hello, World!";

    // 2. "s" is an lvalue expression because it has a name and refers to a specific object in
    // memory.
    std::cout << "Address of s: " << &s << std::endl;
